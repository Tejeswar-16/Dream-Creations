...
export default function ComboCheckout(){
        ...
        async function fetchCombo(){
            try{
                ...
                const q = query(
                    ...
                    where("comboId","==",Number(params.comboId))
                )
    ...
    function generateUPIQR({upiId,name,amount}){
        console.log(upiLink);
        return `upi://pay?pa=${upiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent("Ordered "+combo.comboName)}`
    }
    ...
    return (
        <>
            <div className="relative bg-gradient-to-b from-purple-100 via-purple-100 to-blue-100 py-1 min-h-screen">
                <NavBar/>
                <div className="select-none flex flex-col gap-y-4 md:flex-row mx-auto border-3 border-blue-700 rounded-xl shadow-lg shadow-blue-900 p-4 mb-10 bg-gradient-to-br from-blue-100 via-purple-200 to-purple-100 w-75 md:w-190 lg:w-250 xl:w-350">
                    <div className="flex-1 lg:p-2 mr-4">
                                ...
                                <div className="flex flex-col gap-y-2">
                                    {
                                        combo.products &&
                                            <>
                                                <Image onClick={() => router.push("/products/"+combo.products[0].productId)} className="rounded-xl hover:cursor-pointer" src={combo.products[0].productImage} width={70} height={50} alt="prod-image"></Image>
                                                <h1 className="font-sans text-blue-900">+{combo.products.length-1} more...</h1>
                                            </>
                                    }
                                </div>
                                <div className="flex flex-col gap-y-1">
                                    <h1 className="font-sans text-blue-900 font-bold text-lg lg:text-xl">{combo.comboName}</h1>
                                    {
                                        combo.products && 
                                        <h1 className="font-sans text-blue-900 text-justify">{combo.products.map((p) => p.productName).join(", ")}</h1>
                                    }
                        ...
                        <div className="flex flex-row justify-between mt-4 items-center">
                            ...
                            <h1 className="font-sans text-blue-900 font-semibold text-lg md:mr-20">₹{(Number(combo.comboPrice)*quantity).toFixed(2)}</h1>
                        </div>
                        ...
                        <div className="flex flex-row justify-between items-center mt-4">
                            ...
                            <h1 className="font-sans text-blue-900 font-bold text-xl md:mr-20">₹{(Number(combo.comboPrice*quantity)+40).toFixed(2)}</h1>
                        </div>
                    </div>
                            ...
                            proceed && 
                                <div className="hidden sm:block border border-fuchsia-900 p-2 rounded-xl mt-4">
                                    ...
                                    <div className="flex flex-col justify-center items-center">
                                        ...
                                        <p className="select-none text-sm font-sans text-blue-900">Please scan the QR code below to pay ₹{(Number(combo.comboPrice*quantity)+40).toFixed(2)}</p>
                                        <div className="mt-2 flex flex-row gap-x-1 items-center">
                                            ...
                                            <p className="select-none text-lg font-sans font-bold text-fuchsia-900">₹{(Number(combo.comboPrice*quantity)+40).toFixed(2)}</p>
                                        </div>
                                        {     
                                            qrImage && 
